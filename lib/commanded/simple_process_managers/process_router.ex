defmodule Commanded.SimpleProcessManagers.ProcessRouter do
  @moduledoc false
  ...
  alias Commanded.SimpleProcessManagers.ProcessManagerInstance
  alias Commanded.SimpleProcessManagers.Supervisor
      ...
      case process_manager_module.interested?(data) do
        ...
        {:continue, process_uuid} ->
          ...
          if ProcessManagerInstance.new?(process_instance) do
            state = stop_process_manager(process_uuid, state)
  ...
  defp handle_routing_error(error, %RecordedEvent{} = failed_event, %State{} = state) do
    ...
    Logger.error(fn ->
      describe(state) <> " routing error: #{inspect(error)} from event: #{inspect(data)}"
    end)
