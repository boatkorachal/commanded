defmodule Commanded.ProcessManagers.ConcurrentProcessRouter do
  @moduledoc false
  ...
  def handle_info({:events, events}, %State{} = state) do
    ...
    Logger.debug(fn ->
      describe(state) <> " received #{describe_events(events)}"
    end)
  ...
  defp confirm_receipt(%RecordedEvent{} = event, %State{} = state) do
    ...
    {to_confirm_receipts, pending_confirm_receipts} =
    ...
    if Enum.count(to_confirm_receipts) > 0 do
      Logger.debug(fn ->
        describe(state) <>
          " confirmings #{describe_events(to_confirm_receipts)} pendings #{
            describe_events(pending_confirm_receipts)
          }"
      end)

      last_to_confirm_event = List.last(to_confirm_receipts)
      :ok = Subscription.ack_event(subscription, last_to_confirm_event)
      ...
      %State{
        state
        | pending_confirm_receipts: pending_confirm_receipts,
          last_seen_event: last_to_confirm_event.event_number
      }
    else
      Logger.debug(fn ->
